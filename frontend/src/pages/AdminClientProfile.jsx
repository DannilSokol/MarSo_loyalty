...
import { ArrowLeft, User, History, Plus, X, Home, RefreshCw } from 'lucide-react'
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8">
            <nav className="flex items-center gap-2 text-sm text-gray-500 mb-8">
                <Link to="/admin" className="hover:text-marso flex items-center gap-1"><Home size={16}/> Панель</Link>
                <span>›</span>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10">
                ...
                <div className="flex gap-3">
                    <button onClick={()=>navigate(-1)} className="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 flex items-center gap-2">
                        <ArrowLeft size={18}/> Назад
                    </button>
                    <button onClick={()=>setShowCreateVisit(true)} className="px-6 py-2.5 bg-marso text-white rounded-xl hover:bg-marso-dark flex items-center gap-2 shadow-sm">
                        <Plus size={18}/> Визит
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm mb-8">
                ...
                <div className="grid md:grid-cols-3 gap-6">
                    <div className="space-y-2"><div className="text-sm text-gray-600">Баланс бонусов</div><div className="text-2xl font-bold text-emerald-700">{client.balance}</div></div>
                    <div className="space-y-2"><div className="text-sm text-gray-600">Всего потрачено</div><div className="text-2xl font-bold">{client.total_spent}</div></div>
                    <div className="space-y-2"><div className="text-sm text-gray-600">Посещений</div><div className="text-2xl font-bold">{visits.length}</div></div>
                </div>
            </div>

            {/* История посещений */}
            <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
                <h2 className="text-xl font-bold mb-6 flex items-center gap-3"><History size={20}/> История посещений</h2>
                {visits.length === 0 ? (
